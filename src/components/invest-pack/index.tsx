      ...
      render: (_, row) => (
        <div className="flex flex-col gap-1">
          <div className="flex gap-2">
            <label>Tên gói:</label>
            <div className="font-[700]">{row?.name}</div>
          </div>
          <div className="flex gap-2">
            <label>VIP:</label>
            <div>{row?.vip}</div>
          ...
          <div className="flex gap-2">
            <label>Tiến độ</label>
            <div>{row?.earningDay}%</div>
          </div>
          <div className="flex gap-2">
            <label>Lợi nhuận</label>
            <div className="font-[500]">{row?.incomePerDay}%</div>
          </div>
          <div className="flex gap-2">
            <label>Quy mô</label>
          ...
          {/* <div className="flex gap-2">
            ...
            <div>
              {row?.soldOutAt ? new Date(row?.soldOutAt).toLocaleString() : '-'}
            </div>
          </div> */}
        </div>
      ),
      ...
      render: (_, row) => (
        <div className="flex gap-4 justify-center">
          <div className="flex flex-col items-center gap-2 mb-4">
            ...
            {row?.urlImage ? (
              <img src={row.urlImage} width={100} alt="Ảnh mô tả" />
            ) : (
              <div>Chưa có ảnh</div>
            )}
          </div>
    ...
    <BasePageContainer breadcrumb={breadcrumb}>
      ...
      >
        <Form form={form} onFinish={onFinish} layout="vertical">
          ...
          <Form.Item name="vip" label="Vip" rules={[{ required: true }]}>
            <Select>
          ...
          <Form.Item
            name="price"
            label="Giá"
            rules={[{ required: true, message: 'Vui lòng nhập giá min' }]}
          >
          ...
          <Form.Item
            name="earningDay"
            label="% tiến độ"
            rules={[{ required: true, message: 'Vui lòng nhập số % tiến độ' }]}
          >
          ...
          >
            ...
            <Upload {...uploadProps('urlImage')}>
              <Button
                icon={
                  loadingMap.urlImage ? <LoadingOutlined /> : <UploadOutlined />
                }
              >
                Upload ảnh chính
              </Button>
          ...
          >
            ...
            <Upload {...uploadProps('desImage')}>
              <Button
                icon={
                  loadingMap.desImage ? <LoadingOutlined /> : <UploadOutlined />
                }
              >
                Upload ảnh mô tả
              </Button>
          ...
          <Form.Item name="inventory" label="Quy mô dự án">
            <InputNumber min={0} style={{ width: '100%' }} />
          </Form.Item>
